...
app.get("/api/items", async (req, res) => {
  ...
  if (isDbMode) {
    ...
    if (q) { vals.push(`%${q.toLowerCase()}%`); clauses.push(`(lower(name) like $${vals.length} or lower(description) like $${vals.length})`); }
    if (min) { vals.push(Number(min)); clauses.push(`(price is null or price >= $${vals.length})`); }
    if (max) { vals.push(Number(max)); clauses.push(`(price is null or price <= $${vals.length})`); }
    const where = clauses.length ? `where ${clauses.join(" and ")}` : "";
    const { rows } = await pool.query(`select * from items ${where} order by name asc`, vals);
...
app.post("/api/items", async (req, res) => {
  ...
  if (isDbMode) {
    const q = `insert into items (id,type,name,category,subcategory,price,stock,image,description)
               values ($1,$2,$3,$4,$5,$6,$7,$8,$9) returning *`;
    const vals = [item.id, item.type, item.name, item.category, item.subcategory || null,
                  item.price ?? null, item.stock ?? null, item.image || null, item.description || null];
...
app.put("/api/items/:id", async (req, res) => {
  ...
  if (isDbMode) {
    ...
    const q = `update items set type=$2,name=$3,category=$4, subcategory=$5, price=$6, stock=$7, image=$8, description=$9
               where id=$1 returning *`;
    const vals = [next.id,next.type,next.name,next.category,next.subcategory||null,next.price??null,next.stock??null,next.image||null,next.description||null];
    const { rows } = await pool.query(q, vals);
    return res.json(rows[0]);
  } else {
...
app.get("/api/categories", async (_req, res) => {
  if (isDbMode) {
    const { rows } = await pool.query(`select distinct category from items where type='product' order by category asc`);
    return res.json(rows.map(r => r.category));
  } else {
    const cats = Array.from(new Set(db.items.filter(i => i.type === "product").map(i => i.category))).sort();
    return res.json(cats);
  }
});

app.get("/api/health", (_req, res) => res.json({ ok: true, mode: isDbMode ? "db" : "json" }));
